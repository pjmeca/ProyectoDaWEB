...
export default function Estrellas({ calificacion }) {
  ...
  if(decimal<0.25 && num)
    //Vacia
    estrellas.push(<FontAwesomeIcon icon="fa-regular fa-star" key={calificacionEntera+1}/>);
  else if(decimal>0.75 && num)
    //Entera
    estrellas.push(<FontAwesomeIcon icon="fa-solid fa-star" key={calificacionEntera+1}/>);
  else if(num)
    //Media
    estrellas.push(<FontAwesomeIcon icon="fa-solid fa-star-half-stroke" key={calificacionEntera+1}/>);
