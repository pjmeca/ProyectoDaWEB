...
import { GetJWT, GetCorreo, GetSub } from "../../utils/JWT";
...
export default function TablaPlatos({
    ...
    return platos.map((plato) =>
      editable || plato.disponible ? (
        <tr key={plato.nombre}>
          ...
          <th style={editable ? { width: "45%" } : { width: "65%" }}>
            {" "}
            {plato.descripcion}
          </th>
          ...
          {editable ? (
            ...
            <th>
              <Button
                className={"edicion incidencia"}
                variant="link"
                onClick={() => {
                  setPlatoActual(plato);
                  setComentarioIncidencia("");
                  setModalIncidenciaShow(true);
                }}
              >
  ...
  function handleIncidencia() {
    if (
      platoActual.nombre.trim() === "" ||
      comentarioIncidencia.trim() === ""
    ) {
    ...
    fetch(`/incidencias`, {
      ...
      .then((response) => {
        if (response.ok) history.go(0);
          ...
          throw new Error(
            "Error al crear la incidencia, por favor, inténtalo más tarde."
          );
      })
      .catch((error) => {
            ...
            body={
              <div>
                ...
                >
                  ...
                  <Form.Control
                    type="text"
                    value={platoActual ? platoActual.nombre : ""}
                    disabled
                  />
                </Form.Group>
                ...
                >
                  ...
                  <Form.Control
                    ...
                    onChange={(event) =>
                      setComentarioIncidencia(event.target.value)
                    }
                  />
                </Form.Group>
